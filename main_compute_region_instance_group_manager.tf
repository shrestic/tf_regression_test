...
#   name    = "${local.cluster_name}-cluster-regional-mig-${split("~", each.key)[0]}"

#   base_instance_name        = lower("${local.cluster_name}-cluster-regional-mig-${split("~", each.key)[0]}")
#   region                    = "us-central1"
...
#   lifecycle {
#     precondition {
#       condition     = data.google_compute_region_instance_template.head_vm.disk[0].source_image == data.google_compute_instance_template.worker_node[each.key].disk[0].source_image
